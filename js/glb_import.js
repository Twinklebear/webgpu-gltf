...
var GLTFPrimitive =
    ...
    GLTFPrimitive.prototype.buildRenderBundle =
        function(device,
                 ...
                 swapChainFormat,
                 depthFormat) {
    var vertexShader = shaderModules.posVert;
    var fragmentShader = shaderModules.posFrag;
    ...
    if (this.normals) {
        vertexShader = shaderModules.posNormalVert;
        fragmentShader = shaderModules.posNormalFrag;
    ...
    if (this.texcoords.length > 0) {
        if (this.normals) {
            vertexShader = shaderModules.posNormalUVVert;
            fragmentShader = shaderModules.posNormalUVFrag;
        } else {
            vertexShader = shaderModules.posUVVert;
            fragmentShader = shaderModules.posUVFrag;
        }
    ...
    if (this.material.baseColorTexture) {
        vertexShader = shaderModules.pnuTexturedVert;
        fragmentShader = shaderModules.pnuTexturedFrag;
    }
...
var GLTFMaterial =
    ...
    if (this.baseColorTexture) {
        // Defaults for sampler and texture are fine, just make the objects exist to pick them
        // up
...
var uploadGLBModel = async function(buffer, device) {
    ...
    if (glbJsonData["images"] !== undefined) {
        for (var i = 0; i < glbJsonData["images"].length; ++i) {
            ...
            // and find if the texture which uses this image is used by a metallic/roughness
            // param
